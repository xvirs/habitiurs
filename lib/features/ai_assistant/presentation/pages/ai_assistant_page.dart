...
class _LoadedView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final recommendation = AIRecommendationSection(
    ...
    final educational = EducationalContentSection(
      content: state.educationalContent,
    );
    final guides = AppGuideSection(guides: state.appGuides);

    final isWide = Responsive.isWide(context);

    final Widget content = isWide
        // Pantalla ancha: recomendación a la izquierda, material a la derecha.
        ? Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Expanded(child: recommendation),
              Expanded(
                child: Column(children: [educational, guides]),
              ),
            ],
          )
        // Teléfono: columna única.
        : Column(children: [recommendation, educational, guides]);
